...
export function Marquee({
  children, className, duration = 20, pauseOnHover = false,
  direction = "left", fade = true, fadeAmount = 10, ...props
}: MarqueeProps) {
  ...
  return (
    <>
      <style>{`
        ...
        .marquee-scroller {
          display: flex;
          animation: ${isVertical ? (direction === "up" ? "scroll-y" : "scroll-y-reverse") : (direction === "left" ? "scroll" : "scroll-reverse")} ${duration}s linear infinite;
        }
        .marquee-scroller.paused { animation-play-state: paused; }
        @media (prefers-reduced-motion: reduce) { .marquee-scroller { animation: none; } }
      `}</style>
      <div
        ref={containerRef}
      ...
      >
        <div className={cn("marquee-scroller flex shrink-0", isVertical && "flex-col", isPaused && "paused")}>
          ...
          {items.map((item, i) => (
            <div key={`b-${i}`} className={cn("flex shrink-0", isVertical && "w-full")} aria-hidden>{item}</div>
          ))}
        </div>
      </div>
    </>
